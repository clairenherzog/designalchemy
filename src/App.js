...
export default function DesignAlchemyLab() {
  ...
  // Generate rising bubbles and particles
  useEffect(() => {
    const interval = setInterval(() => {
      setBubbles(prev => {
        const newBubbles = {};
        testTubes.forEach(tube => {
          newBubbles[tube.id] = Math.random();
        });
        return newBubbles;
      });
      ...
      setParticles(prev => {
        const newParticles = {};
        testTubes.forEach(tube => {
          newParticles[tube.id] = Math.random();
        });
        return newParticles;
      });
    }, 2000);
                    ...
                    <div className="relative w-56 h-72 flex items-center justify-center overflow-hidden bottle-container">
                      ...
                      {flask.filled && (
                        <div 
                          ...
                          style={{
                            background: `linear-gradient(180deg, ${flask.colors[0]} 0%, ${flask.colors[1]} 100%)`,
